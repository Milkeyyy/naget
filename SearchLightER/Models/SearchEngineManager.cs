...
public static class SearchEngineManager
{
	...
	{
		_engineCollection = new SearchEngineList();
		_engineCollection.List = [
	...
	{
		var data = JsonSerializer.Serialize(_engineCollection);
	...
	public static void Load()
	{
		...
		{
			data = JsonSerializer.Deserialize<SearchEngineList>(File.ReadAllText(FilePath));
			if (data != null) _engineCollection = data;
			else Create(); // null の場合は新規作成する
		}
		// 存在しない場合は新規作成する
		else
		{
	...
	/// 指定されたIDの検索エンジンを取得する
	/// </summary>
	/// <param name="id">取得する検索エンジンのID</param>
	...
	/// 指定されたIDの検索エンジンを削除する
	/// </summary>
	/// <param name="id">削除する検索エンジンのID</param>
	...
	/// </summary>
	/// <param name="id">名前を変更する検索エンジンのID</param>
	...
	public static void ChangeUri(string id, string newUri)
	{
		var result = Get(id);
		if (result != null)
		{
