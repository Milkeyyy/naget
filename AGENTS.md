...
- NUKE のエントリポイントは `build/Build.cs`、ターゲットは `Compile`
...
### MVVM (Epoxy)
- `Epoxy.Avalonia11` 使用、`[ViewModel]` 属性でVMを宣言
...
### グローバル状態
`App.axaml.cs` が全ウィンドウ、Logger、Updater、WindowService を静的シングルトンで管理。全コンポーネントが `App.XXX` を直接参照する。

### 起動フロー
`Program.cs` → `VelopackApp.Build().Run()` → `BuildAvaloniaApp().StartWithClassicDesktopLifetime()`
→ `App.Initialize()` (Config/Locale/SearchEngine 読み込み) → `App.OnFrameworkInitializationCompleted()` (各Window作成、HotKey開始、Updater開始)

### 主要ディレクトリ
| パス | 役割 |
|------|------|
| `naget/Views/` | AXAML UI定義 |
| `naget/ViewModels/` | `[ViewModel]` 付きVMクラス |
| `naget/Models/` | 設定、検索エンジンモデル |
| `naget/Helpers/` | ホットキー(SharpHook)、ロガー、Velopack更新 |
| `naget/Services/` | WindowService (ウィンドウ表示制御) |
| `naget/CEF_Resources/` | WebView用CEFバイナリ (win-x64/win-arm64) |
...
- インデント: **タブ** / 改行: **CRLF** / 文字コード: **UTF-8-BOM**
- **常に明示的な型を使用 (`var` 禁止)**
- `this.` 修飾は使用しない
...
- 更新は Velopack (`vpk` CLI) で管理、チャネル名は `{os}-{arch}-{channel}`
- `.mcp.json` に Avalonia ドキュメント MCP が定義済み
- **`Build_env.txt` はシークレットを含むため絶対にコミット・操作しないこと**
- Roslynator アナライザーが有効 (`roslynator.analyzers`)
